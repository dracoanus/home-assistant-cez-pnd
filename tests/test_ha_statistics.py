...
class StatisticsTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_cumulative_sums_are_monotonic_and_corrections_shift_later_sums(self) -> None:
        start = datetime(2026, 9, 1, tzinfo=UTC)
    ...
    async def test_history_stops_exactly_at_global_count_and_rejects_overcount(self) -> None:
    ...
    async def test_same_revision_skips_and_expected_growth_forces_full_scan(self) -> None:
    ...
    async def test_revision_change_uses_recent_window_and_removes_stale_hour(self) -> None:
        old = _status("ds_old", expected=4)
        new = _status("ds_new", expected=4)
        start = datetime(2026, 9, 10, tzinfo=UTC)
        missing = tuple(
            _measurement(start + timedelta(minutes=15 * index), value=None, quality="missing")
            for index in range(4)
        )
    ...
    async def test_historical_correction_rebuilds_later_cumulative_sums(self) -> None:
        old = _status("ds_old", expected=8)
        new = _status("ds_new", expected=8)
        start = datetime(2026, 9, 10, tzinfo=UTC)
        client = _Client(
            new,
            [_page(new, 4, _hour(start, value="2"))],
        )
        imported: dict[str, list[dict]] = {}
        ...
        manager._last_revision = old.dataset_revision
        manager._last_expected_count = 8
        manager._hourly = {
            "grid_import": {
                start: Decimal("4"),
        ...
        self.assertEqual([row["state"] for row in rows], [8.0])
        self.assertEqual([row["sum"] for row in rows], [8.0])
